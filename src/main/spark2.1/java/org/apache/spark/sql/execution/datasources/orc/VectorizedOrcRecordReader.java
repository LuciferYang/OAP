...
/**
 * This class is a copy of VectorizedOrcRecordReader with minor changes to be able to
...
public class VectorizedOrcRecordReader extends RecordReader<Void, ColumnarBatch> {
  ...
  private org.apache.spark.sql.execution.vectorized.ColumnVector[] columnVectors;

  // The wrapped ORC column vectors. It should be null if `copyToSpark` is true.
  private org.apache.spark.sql.execution.vectorized.ColumnVector[] orcVectorWrappers;
  ...
  public void initBatch(
    ...
    if (copyToSpark) {
      columnarBatch = ColumnarBatch.allocate(resultSchema, MEMORY_MODE);

      int numCols = resultSchema.length();

      columnVectors =
        new org.apache.spark.sql.execution.vectorized.ColumnVector[numCols];

      for (int i = 0; i < numCols; i++) {
        columnVectors[i] = columnarBatch.column(i);
      }
      ...
      columnarBatch = ColumnarBatch.allocate(resultSchema, MemoryMode.ON_HEAP);
      orcVectorWrappers =
        new org.apache.spark.sql.execution.vectorized.ColumnVector[resultSchema.length()];
      ...
      for (int i = 0; i < requiredFields.length; i++) {
        ...
        // Initialize the missing columns once.
        if (colId == -1) {
          OnHeapColumnVector missingCol = (OnHeapColumnVector)columnarBatch.column(i);
          missingCol.putNulls(0, CAPACITY);
          missingCol.setIsConstant();
          orcVectorWrappers[i] = missingCol;
        } else {
          OrcColumnVector column = new OrcColumnVector(dt, batch.cols[colId]);
          orcVectorWrappers[i] = column;
          columnarBatch.setColumn(i,column);
        }
      }
      ...
      if (partitionValues.numFields() > 0) {
        ...
        for (int i = 0; i < partitionValues.numFields(); i++) {
          OnHeapColumnVector partitionCol =
            (OnHeapColumnVector)columnarBatch.column(i + partitionIdx);
  ...
  protected boolean readToColumnVectors(int batchSize) {
    ...
    for (int i = 0; i < requiredFields.length; i++) {
      ...
      org.apache.spark.sql.execution.vectorized.ColumnVector toColumn = columnVectors[i];
  ...
  private void putRepeatingValues(
      ...
      org.apache.spark.sql.execution.vectorized.ColumnVector toColumn) {
  ...
  private void putNonNullValues(
    ...
    if (type instanceof BooleanType) {
      ...
      BytesColumnVector data = ((BytesColumnVector)fromColumn);
      org.apache.spark.sql.execution.vectorized.ColumnVector arrayData = toColumn.arrayData();
      int totalNumBytes = 0;
      for (int index = 0; index < batchSize; index++) {
